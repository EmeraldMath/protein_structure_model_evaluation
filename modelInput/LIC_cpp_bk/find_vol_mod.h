...
void voltetra_ext(int* idT, double* V_atom, int* res, double** LIS_res, double* S_lag_int,
                  edge_type* edgelist, tri_type* reftri, double** coords, double** char_pt_T);

void unravel(int& num_ring_in, int* ring_in, int* adj_tri, int& num_ring_out, int* ring_out, int& bit,
             int& previous, int* idT);

void voltetra(int& num_int_edge, int* int_edge_list, edge_type* refedg, double** coords, double** char_pt_T,
              double* V_atom, double* S_lag_int, int* res, double** LIS_res, double* lift);

void compare_vol(int& num_coord, double& V_total, double* V_atom);

void voltri(double& V_total, double* V_atom, double* S_atom, int* res, double** LIS_res, double* S_lag_int,
            double** coords, double* weights, tri_type* trilist, double *S_lag2);

void voledg(double** coords, double* weights, edge_type* edgelist, int* idT, double& V_total, double* V_atom,
            double* S_atom, double* S_lag_int, double** LIS_res, int* res, int **T);

void volver(int& num_ver_ext, int& maxupver, int* ver_ext, ver_type* verlist, double* weigths, double** coords,
            double& V_total, double* V_atom, double* S_atom, double* S_lag_int, int *idT, int **T);

void classify_ver(int& num_coord, int& num_ver_ext, double* weights, ver_type* verlist, int* ver_ext,
                  edge_type *refedg, double **coords);

void classify_edg(int& num_edg_ext, int& num_int_edge, int& num_edg, edge_type* edgelist, edge_type* refedg, double** coords,
                  double* lift, double* weights, tri_type* reftri, int* int_edg_list);

void classify_tri(int* alfT, int& num_tri_ext, int &num_edg, int& num_tri, tri_type* reftri, tri_type* trilist, double** coords,
                  double* lift, double* weights);

void extract_edg_tri(int** atri, int** aedg, int num_T, int num_coord, ver_type* verlist, tri_type* reftri,
                     edge_type* refedg, int** T, int** T_nbrs, int &num_tri, int& num_edg);
void tetra_ring(int& edg_ind, int edg_num_int, int T_ind, edge_type* refedg, int** T, int** Tnbr, int** atri, int& num_tri,
                tri_type* reftri, int** aedg);

void mark_edg_tri(int &edg_ind, int &tag, int &o_ver, int &tetra_ind, int &edg_int, edge_type *refedg, int **T,
                  tri_type *reftri, int **aedg, int **atri);
...
void find_vol_intersection(int& num_coord, double** coords, double* weights, int& num_res, int* res,
                           double* V_atom, double* V_res, double* S_atom, double* S_lag_int, double** LIS_res) {
    ...
    voledg(coords, weights, edgelist, idT, V_total, V_atom, S_atom, S_lag_int, LIS_res, res, T);
    voltri(V_total, V_atom, S_atom, res, LIS_res, S_lag_int, coords, weights, trilist, S_lag2);
    voltetra_V(V_total, num_T2, coords, T2);
    voltetra(num_int_edge, int_edge_list, refedg, coords, char_pt_T, V_atom, S_lag_int, res, LIS_res, lift);
    voltetra_ext(idT, V_atom, res, LIS_res, S_lag_int, edgelist, reftri, coords, char_pt_T);

    for (int i = 0; i < num_coord; ++ i) {
        V_res[res[i]-1] += V_atom[i];
    }

    compare_vol(num_coord, V_total, V_atom);
    std::cerr << "V_total " << std::setprecision(17) << V_total << std::endl;
    ...
    while (current) {
        ...
        if (num_ring2 != 0) {
            ...
            if (res[e1-1] != res[e2-1]) {
                LIS_res[res[e2-1]-1][res[e1-1]-1] += surf;
                LIS_res[res[e1-1]-1][res[e2-1]-1] += surf;
            }
        }
...
void voltri(double& V_total, double* V_atom, double* S_atom, int* res, double** LIS_res, double* S_lag_int,
    ...
    while(current) {
	    ...
	    if (res[i1-1] != res[i2-1]) {
           LIS_res[res[i2-1]-1][res[i1-1]-1] -= S12_lag;
           LIS_res[res[i1-1]-1][res[i2-1]-1] -= S12_lag;
	    }

	    if (res[i1-1] != res[i3-1]) {
	       LIS_res[res[i3-1]-1][res[i1-1]-1] -= S13_lag;
	       LIS_res[res[i1-1]-1][res[i3-1]-1] -= S13_lag;
	    }

	    if (res[i2-1] != res[i3-1]) {
	       LIS_res[res[i3-1]-1][res[i2-1]-1] -= S23_lag;
           LIS_res[res[i2-1]-1][res[i3-1]-1] -= S23_lag;
        }
	    current = current->next_ext;
...
void voledg(double** coords, double* weights, edge_type* edgelist, int* idT, double& V_total, double* V_atom,
    ...
    while (current) {
        ...
        S_atom[e2-1] -= S2_sphr;
        if (res[e1-1] != res[e2-1]) {
            LIS_res[res[e2-1]-1][res[e1-1]-1] += S_lag;
            LIS_res[res[e1-1]-1][res[e2-1]-1] += S_lag;
        }
...
void volver(int& num_ver_ext, int& maxupver, int* ver_ext, ver_type* verlist, double* weights, double** coords,
            double& V_total, double* V_atom, double* S_atom, double* S_lag_int, int* idT, int** T) {
